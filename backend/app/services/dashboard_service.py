...
class DashboardService:
    ...
    def get_summary(cls, *, db: Session, start_date=None, end_date=None, severity=None, source=None, status=None):
        ...
        high_risk_filters = [*filters, Alert.risk_score >= 61, NormalizedLog.src_ip.is_not(None)]
        source_filter = None
        if source:
            source_filter = (NormalizedLog.source == source) | (NormalizedLog.source_type == source)
        direct_ips = (
            select(NormalizedLog.src_ip.label("src_ip"))
            .join(Alert, Alert.normalized_log_id == NormalizedLog.id)
            .where(*high_risk_filters)
        )
        related_ips = (
            select(NormalizedLog.src_ip.label("src_ip"))
            .join(AlertRelatedLog, AlertRelatedLog.normalized_log_id == NormalizedLog.id)
            .join(Alert, AlertRelatedLog.alert_id == Alert.id)
            .where(*high_risk_filters)
        )
        if source_filter is not None:
            direct_ips = direct_ips.where(source_filter)
            related_ips = related_ips.where(source_filter)
        ip_rows = union_all(direct_ips, related_ips).subquery()
        high_risk_ip_count = db.execute(select(func.count(func.distinct(ip_rows.c.src_ip)))).scalar_one()
        return {
